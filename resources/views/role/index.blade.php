                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    ...
                                    <th class="py-4 px-6" style="width:500px">
                                        <button id="showPerIcon{{ $role->id }}"
                                            onclick="PermissionShow('show',{{ $role->id }})" type="button"
                                            data-tooltip-target="show-button" data-bs-toggle="tooltip"
                                            data-bs-placement="top">
                                            <x-svg.eye class="w-6 h-6 text-pink-400" />
                                        </button>
                                        <button class="hidden" id="hidePerIcon{{ $role->id }}"
                                            onclick="PermissionShow('hide',{{ $role->id }})" type="button"
                                            data-tooltip-target="hide-button" data-bs-toggle="tooltip"
                                            data-bs-placement="top">
                                            <x-svg.eye-off class="w-6 h-6 text-pink-400" />
                                        </button>
                                        <div id="permission{{ $role->id }}"
                                            class="hidden flex gap-4 flex-wrap text-center">
